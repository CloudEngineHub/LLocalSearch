...
func startAgentChain(ctx context.Context, outputChan chan<- utils.HttpJsonStreamElement, clientSettings utils.ClientSettings) error {
	...
	if sessions[session] == nil {
		...
		sessions[session] = memory.NewConversationWindowBuffer(clientSettings.ContextSize)
		memory.NewChatMessageHistory()

		sessions[session].ChatHistory.AddMessage(ctx, llms.SystemChatMessage{
		...
		// utils.LoadLocalFiles(clientSettings.Session, clientSettings.ChunkSize, clientSettings.ChunkOverlap)
		// slog.Info("Local files loaded")
	}
